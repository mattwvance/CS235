...
void parse_instruction(std::string temp, std::ofstream &ofs, ExpressionManager *aptr)
{
	...
	{ // command to add a new node as a head node
		if (aptr->isBalanced(ss.str().substr(11, std::string::npos)))
		{ // use the rest of the stringstream as isBalanced input
	...
	else if (command == "postfixToInfix")
	{ // command to remove a fighter from the arena (given the fighters name)
		expression = aptr->postfixToInfix(ss.str().substr(15, std::string::npos));
		ofs << temp << "\n  " << expression << std::endl;
	}
	else if (command == "postfixEvaluate")
	{ // command to return a pointer to a fighter (given the name of the fighter)
